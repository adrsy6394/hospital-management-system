      ...
      <div className="space-y-4 md:space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-sm md:text-base text-gray-600 mt-1">View and manage hospital staff records</p>
          </div>
          <button 
            onClick={() => navigate('/admin/staff/add')}
            className="w-full sm:w-auto px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-semibold text-sm md:text-base flex items-center justify-center space-x-2"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs md:text-sm font-medium text-gray-500">Total Staff</p>
                <p className="text-2xl md:text-3xl font-bold text-gray-900 mt-1">{staffList.length}</p>
              </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 md:p-4">
          <div className="relative">
            ...
            <input
              type="text"
              className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-sm md:text-base outline-none"
          ...
          <div className="overflow-x-auto">
            <table className="min-w-[800px] md:min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th scope="col" className="px-4 md:px-6 py-3 md:py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    Staff Info
                  </th>
                  <th scope="col" className="px-4 md:px-6 py-3 md:py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    Role
                  ...
                  <th scope="col" className="px-4 md:px-6 py-3 md:py-4 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredStaff.map((staff) => (
                  <tr key={staff._id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 md:px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="ml-3 md:ml-4 min-w-0">
                          <div className="text-sm font-medium text-gray-900 truncate max-w-[120px] md:max-w-[200px]">{staff.userId?.name}</div>
                          <div className="text-xs md:text-sm text-blue-600">{staff.staffId}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 md:px-6 py-4 whitespace-nowrap">
                      <div className="text-xs md:text-sm text-gray-900 font-medium">{staff.position}</div>
                      <div className="text-[10px] md:text-xs text-gray-500">{staff.department}</div>
                    </td>
                    <td className="px-4 md:px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-[10px] md:text-xs leading-5 font-semibold rounded-full capitalize 
                    ...
                    <td className="px-4 md:px-6 py-4 whitespace-nowrap">
                      <div className="text-xs md:text-sm text-gray-900">{staff.userId?.phone}</div>
                      <div className="text-[10px] md:text-xs text-gray-500 truncate max-w-[120px] md:max-w-none">{staff.userId?.email}</div>
                    </td>
