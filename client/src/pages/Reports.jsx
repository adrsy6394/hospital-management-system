    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Hospital Analytics & Reports</h1>
            <p className="text-gray-600 mt-1">Real-time overview of hospital operations and finances</p>
          </div>
          <button 
            onClick={() => window.print()}
            className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition font-medium flex items-center shadow-sm"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 border-l-4 border-l-indigo-500">
            ...
            <p className="text-2xl font-bold text-gray-900">{reportData.totalPatients}</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 border-l-4 border-l-green-500">
            ...
            <p className="text-2xl font-bold text-gray-900">{reportData.totalDoctors}</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 border-l-4 border-l-yellow-500">
            ...
            <p className="text-2xl font-bold text-gray-900">{reportData.totalStaff}</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 border-l-4 border-l-purple-500">
            <p className="text-sm font-medium text-gray-500 mb-1">All Appointments</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="p-0">
              <ul className="divide-y divide-gray-200">
                {reportData.recentAppointments.length > 0 ? reportData.recentAppointments.map(app => (
                  <li key={app._id} className="p-5 hover:bg-gray-50 transition">
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="text-sm font-medium text-gray-900">Patient: {app.patientId?.userId?.name || 'Unknown'}</p>
                        <p className="text-sm text-gray-500 mt-1">Dr. {app.doctorId?.userId?.name || 'Unknown'}</p>
                      </div>
                      <div className="text-right">
                        <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium capitalize shadow-sm
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200">
            <div className="p-5 border-b border-gray-200 bg-gray-50 rounded-t-xl flex justify-between items-center">
              <h2 className="text-lg font-bold text-gray-900">Recent Revenue Transactions</h2>
            </div>
            <div className="p-0">
              <ul className="divide-y divide-gray-200">
                {reportData.recentBills.length > 0 ? reportData.recentBills.map(bill => (
                  <li key={bill._id} className="p-5 hover:bg-gray-50 transition">
                    <div className="flex justify-between items-center">
                      <div className="flex items-center">
                        <div className={`w-2 h-10 rounded-full mr-3 ${bill.paymentStatus === 'paid' ? 'bg-green-500' : 'bg-yellow-400'}`}></div>
                        <div>
                          <p className="text-sm font-medium text-gray-900">Bill: {bill.billId}</p>
                          <p className="text-xs text-gray-500 mt-0.5">{bill.patientId?.userId?.name || 'Unknown Patient'}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-bold text-gray-900">₹{bill.totalAmount.toLocaleString()}</p>
                        <p className={`text-xs font-semibold capitalize mt-0.5 ${bill.paymentStatus === 'paid' ? 'text-green-600' : 'text-yellow-600'}`}>
                          {bill.paymentStatus}
                        </p>
                      </div>
                    </div>
